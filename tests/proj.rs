...
fn gen_coo() -> Vec<LonLat> {
    ...
    for lon in (0..=360).step_by(30) {
        ...
        for lat in -900..=900 {
            // 180 * 10 = 1800 points
            let mut lon_rad = f64::from(lon).to_radians();
            if lon_rad >= PI.twice() {
                lon_rad = PI.twice() - 1.0e-14;
            }
            let lat_rad = (f64::from(lat) / 10.0).to_radians();
    ...
    for lat in (-90..=90).step_by(15) {
        for lon in 0..=3600 {
            let mut lon_rad = (f64::from(lon) / 10.0).to_radians();
            if lon_rad >= PI.twice() {
                lon_rad = PI.twice() - 1.0e-14;
            }
...
fn test_canonicals_back_and_forth() {
    let mas_in_rad = (1.0 / 3_600_000_f64).to_radians();
    // Conic
    test_canonical_back_and_forth(&Cod::new(), mas_in_rad / 1000.0); //  1 uas
    test_canonical_back_and_forth(
        &Coe::from_params(
            (-20.0 - 70.0).half().to_radians(),
            (-20_f64 + 70.0).abs().half().to_radians(),
        ),
        10.0 * mas_in_rad,
    ); //  10 mas
    test_canonical_back_and_forth(
    ...
    test_canonical_back_and_forth(
        &Zpn::from_params(vec![0.0, 1.0, 0.0, -50.0]).unwrap(),
        10.0 * mas_in_rad,
    ); // 10 mas
    test_canonical_back_and_forth(
        &Zpn::from_params(vec![
            0.050, 0.975, -0.807, 0.337, -0.065, 0.010, 0.003, -0.001,
        ])
        .unwrap(),
        10.0 * mas_in_rad,
    ); // 10 mas
